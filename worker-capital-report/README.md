...
Cloudflare Worker and per-issuer SQLite Durable Objects monitor Strategy (CIK 1050446) and Strive (CIK 1920406). The public feed exposes new SEC 8-K / 8-K/A filings, their acceptance and retrieval timestamps, source-document hashes, and validated reported facts. Separate per-week Durable Objects notify Discord after both weekly filings are published to the feed. No browser session or Streamlit callback is needed.

## Schedule

- Monday **06:45 through 09:29:59 America/New_York**, every 30 seconds; Tuesday at the same time when Monday is a regular federal EDGAR holiday.
- A minute cron on Monday and Tuesday 10:00–14:59 UTC starts and repairs the alarm chain. The Eastern-time gate permits only the actual weekly release day and handles daylight saving automatically.
- One Durable Object per company owns deduplication, retry state and its alarm. Public page views do not cause SEC requests.
- At 09:30 Eastern SEC polling stops. An authenticated manual poll can run outside the window. Exceptional government closures beyond the regular federal calendar require an operator update.
...
The current Strategy parser extracts explicit weekly BTC purchases and sales, ending BTC holdings, common and preferred issuance/repurchase tables, and the two designated dollar liquidity balances when present. Its weekly 8-K does not provide every NAV input or a complete ending common share denominator.
...
| POST | `/api/admin/reprocess` | Refetch and reparse one known recent non-baseline primary 8-K, with optional source HTML for audit |
...
- original discovery time and most recent successful document fetch time;
...
Existing filings are baselined on the first successful scan. Only the three most recent baseline filings from the last 30 days are queued for document retrieval. A repeated accession normally leaves the original timestamps and data untouched. After a parser version upgrade, regular polls also refetch recent non-baseline primary 8-Ks parsed by an older version, limited to 14 days and post-activation discovery. The existing two-document limit and SEC cooldowns apply. Failed upgrades preserve the published receipt and persist a document retry deadline; successful upgrades retain original acceptance/discovery times and send deduplication. Amendments are independent accessions; they are not silently applied over earlier data. Same-version document corrections require authenticated reprocessing; completed documents are not continuously redownloaded.

Recovery uses the existing admin Bearer token and a body such as `{"ticker":"MSTR","accession":"0001193125-26-384402","includeDocument":true}`. It fetches only the stored, validated SEC primary URL, honors SEC cooldowns, and preserves original acceptance and discovery times. The response optionally includes `document.html` and its SHA-256. Failed recovery preserves the published receipt. Repeated successful recovery can refetch a document but does not duplicate its outbox or a confirmed Discord send; changed document hashes receive a new verified candidate. Public reads cannot invoke recovery.
...
The coordinator durably stores candidates and independently reads the same bounded feed projection used by `/api/filings`. Both exact accessions, hashes and trusted SEC URLs must be present. Each receipt must be a non-baseline primary `8-K`, accepted on the same weekly release day within 14 days, with completed retrieval timestamps, nonnegative BTC holdings and at least one explicit nonnegative weekly purchase or sale quantity. A Tuesday after a federal Monday holiday uses that Monday's existing week key. A narrow exception permits omitted holdings when both gross BTC directions are explicitly zero and the dated treasury report also states common issuance shares and proceeds. Holdings are never invented. Every supplied activity field must be valid; BTC-related extraction or reporting-period errors prevent notification. Other incomplete report inputs may remain partial. Amendments, unrelated filings and historical setup records do not qualify.

`NOTIFICATIONS_ACTIVE_AFTER` is the fixed initial activation cutoff, **2026-09-08T00:00:00Z**. Both first discovery and completed retrieval must be on or after it. Preserve this value on future deployments: advancing it would discard eligible pending events. Existing receipts are not automatically replayed; authenticated reprocessing can recover an eligible post-activation receipt. Already-sent records and existing Discord retry deadlines survive the upgrade.
...
Discord requests use `wait=true`, disable mentions and confirm a returned message ID. The combined message links both SEC filings and the report, adds explicit `Bought` and/or `Sold` BTC quantities from the verified feed, and says Streamlit is expected to load that feed when opened. Gross buys and sales stay separate. Financial cards still await reconciliation; no completed browser rendering is claimed. Existing outbox keys, confirmed sends and retry state remain compatible.
...
145 tests run in workerd, including both companies' genuine August 31 and September 8 filings plus synthetic sale-only, combined gross activity, zero holdings, approximate quantities, invalid/cumulative/equity/proceeds exclusions, and Bought/Sold Discord deduplication. The suite also covers holiday release days, explicit zero-activity reports, authenticated recovery, automatic parser upgrades, no-browser publication, atomic receipt/outbox/alarm storage, failed handoffs and busy-alarm recovery after 09:30, shared feed projection checks, concurrent candidate revisions, activation gating, cooldowns, restarts and isolated setup tests.
...
- [Strategy September 8, 2026 weekly 8-K](https://www.sec.gov/Archives/edgar/data/1050446/000119312526384402/mstr-20260831.htm)
- [Strive September 8, 2026 weekly 8-K](https://www.sec.gov/Archives/edgar/data/1920406/000162828026060809/asst-20260908.htm)
- [SEC fair-access rules and public data APIs](https://www.sec.gov/search-filings/edgar-search-assistance/accessing-edgar-data)
- [SEC EDGAR holiday calendar](https://www.sec.gov/submit-filings/filer-support-resources/edgar-calendar)
